...
<body>
    <div class="container">
        ...
        <?php
$connection = mysqli_connect('localhost', 'root', '', 'php-data');

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['checkbox']) && !empty($_POST['checkbox'])) {
        $checkedValues = array_filter($_POST['checkbox']); // Filter out empty values
        if (!empty($checkedValues)) {
            $checkedValue = implode(',', $checkedValues);
            $sql = "INSERT INTO `user`(`name`) VALUES ('$checkedValue')";
            $result = mysqli_query($connection, $sql);
        } else {
            echo '<div class="alert" role="alert">
                    No non-empty values selected.
                </div>';
        }
    } else {
        echo '<div class="alert" role="alert">
